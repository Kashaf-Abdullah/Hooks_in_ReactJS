...
const myNewArray=cleardata.filter((curElm)=>{
  //jis pe click hoga uski id match kregi or jin ki match nai means wo jo click nai huye so wo new array mai store hojaega
  return curElm.id!=id
  ...
  return (
    <>
      ...
      cleardata.map((curElm)=>{return (

// key is unique ==in our bioData id is unique 
<h1 key={curElm.id}>Name:{curElm.name} id: {curElm.id} 
